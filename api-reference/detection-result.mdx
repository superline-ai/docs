...
The `DetectionResult` object contains the outcome of the agent detection process. It's returned by the `detector.finalizeDetection()` and `detector.getCurrentDetectionResult()` methods and provides information about whether the current session is classified as an AI agent.
